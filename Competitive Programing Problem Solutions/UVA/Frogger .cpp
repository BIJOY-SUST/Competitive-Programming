...
vector<pair<int,pair<int,ld> > >edge;
vector<pii>point;
int par[mx];
ld make_dis(int x1,int y1,int x2,int y2){
    ld d=(x1-x2)*(x1-x2);
    ld d2=(y1-y2)*(y1-y2);
    ld d3=sqrt(d+d2);
    return d3;
...
bool cmp(const pair<int,pair<int,ld> > &a, const pair<int,pair<int,ld> > &b){
    return a.S.S<b.S.S;
}
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    while(sfi1(n)){
        ...
        for(int i=0;i<n;i++){
            for(int j=i+1;j<n;j++){
                ld d=make_dis(point[i].F,point[i].S,point[j].F,point[j].S);
                edge.pb({i+1,{j+1,d}});
            }
        }
        ...
        for(int i=0;i<edge.size();i++){
            int pu=find_par(edge[i].F);
            int pv=find_par(edge[i].S.F);
            if(pu!=pv){
                par[pu]=pv;
                if(find_par(1)==find_par(2)){
                    cout<<"Scenario #"<<cs++<<nl;
                    cout<<"Frog Distance = ";
                    cout<<setprecision(3)<<fixed<<edge[i].S.S<<nl<<nl;
                    break;
                }
            }
        }
