...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    block=sqrt(n);
    sort(ob+1,ob+q+1,cmp);
    int curL=1,curR=0,sum=0;
    loop1(i,q){
        int left=ob[i].l,right=ob[i].r;
//        debug(left,right);
        while(curR<right){
            curR++;
            if(dp[arr[curR]]+1==arr[curR] ){
                sum+=1;
            }
            else if(dp[arr[curR]]==arr[curR]){
                sum-=1;
            }
            dp[arr[curR]]++;
        }
        while(curR>right){
            if(dp[arr[curR]]==arr[curR]){
                sum-=1;
            }
            else if(dp[arr[curR]]-1==arr[curR] ){
                sum+=1;
            }
        ...
        while(curL<left){
            if(dp[arr[curL]]==arr[curL]){
                sum-=1;
            }
            else if(dp[arr[curL]]-1==arr[curL] ){
                sum+=1;
            }
            dp[arr[curL]]--;
            curL++;
        ...
        while(curL>left){
            curL--;
            if(dp[arr[curL]]+1==arr[curL] ){
                sum+=1;
            }
            else if(dp[arr[curL]]==arr[curL]){
                sum-=1;
            }
            dp[arr[curL]]++;
        }
