...
void matrix_mul(mat &a,mat &b,int m,int d){
    mat sol=zero;
    loop(i,0,d-1) loop(j,0,d-1) loop(k,0,d-1){
...
mat matrix_exp(mat org,int d,int n,int m){
    mat x=I,y=org;
    while(n){
        if(n&1){
            matrix_mul(x,y,m,d);
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    int d,n,m,arr[20],f[20];
    memset(&zero,0,sizeof(mat));
    I=zero;
    loop(i,0,16){
        I.v[i][i]=1;
    }
    while(scanf("%d%d%d",&d,&n,&m)==3){
        if(d==0) break;
        loop(i,0,d-1) sfi1(arr[i]);
        loop(i,0,d-1) sfi1(f[i]);

        mat org=zero,ami;
        loop(i,0,d-1) org.v[0][i]=arr[i]%m;
        loop(i,1,d-1) org.v[i][i-1]=1;

        ami=matrix_exp(org,d,n-1,m);
        ll ans=0;
        loop(i,0,d-1){
            f[d-i-1]%=m;
            ans+=(ami.v[d-1][i]*f[d-i-1]);
            ans%=m;
        }
        pf("%lld\n",ans);
    }
