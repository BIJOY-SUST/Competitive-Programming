...
ll n;
ll boss(ll p,ll c){
    if(n>p*9&&n<=p*18) return 0;
    for(ll i=2;i<=9;i++) if(boss(p*i,c)==0) return 1;
    return 0;
}
int main(){
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    while(scanf("%lld",&n)!=EOF){
        ...
        else if(n>=2&&n<=9) printf("Stan wins.\n");
        else{
            ll check=boss(1,0);
            if(check==1) printf("Stan wins.\n");
...
/*
    ...
    while(scanf("%lld",&n)!=EOF){
        ...
        while(1){
            p*=9;
            if(p>=n){printf("Stan wins.\n");break;}
            p*=2;
            if(p>=n){printf("Ollie wins.\n");break;}
        }
    }
    return 0;
*/
