...
ll lol(int l,int r,int c){
    ...
    if(c==0){
        ret=INT_MIN;
        for(int i=l;i<=r;i++)
            s+=arr[i],ret=max(ret,s+lol(i+1,r,1));
        s=0;
        for(int i=r;i>=l;i--)
            s+=arr[i],ret=max(ret,s+lol(l,i-1,1));
    }
    else{
        ret=INT_MAX;
        for(int i=l;i<=r;i++)
            s-=arr[i],ret=min(ret,s+lol(i+1,r,0));
        s=0;
        for(int i=r;i>=l;i--)
            s-=arr[i],ret=min(ret,s+lol(l,i-1,0));
    }
    return ret;
}
int main(){
