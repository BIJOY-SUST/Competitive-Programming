...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        ll sum=accumulate(all(no),0);
        case_print;
        if(sum==N){
            double ans=0.0;
            for(auto x:no){
                ans+=round(((double)x/N*1.0)*100.0);
            }
            ll ans2=(ll)ans;
            pf("%ld\n",ans2);
        }
            ...
            // ist part
            double d=round(((double)1/N*1.0)*100.0);
            double rem=N-sum;
            rem=d*rem*1.0;
            // second part
            int k=minv(no);
            double d2=round(((double)(k+(N-sum))/N*1.0)*100.0);
            double ans=0.0;
            int f=0;
            for(auto x:no){
                if(x==k&&f==0){
                ...
                else ans+=round(((double)x/N*1.0)*100.0);
            }
            //last part
            //deb(sum,k);
            //deb(rem,d2,ans);
            double d3=round(((double)k/N*1.0)*100.0);
            double hh=max(rem+d3,d2);
            if(hh==d2){
                //cout<<"ami"<<endl;
                ans+=hh;
                ll ans2=(ll)ans;
