...
stack<int>topsort;
int low[mx],disc[mx],scc[mx],hisab[mx],dp[mx];
bool vis[mx];
int cnt,ans,cur_source,d;
...
int dfs2(int u){
    ...
    for(int i=0;i<edge2[u].size();i++){
        int v=edge[u][i];
        ret = max(ret,dfs2(v)+hisab[scc[u]]);
    }
    return ret;
}
int main(){
...
//        for(int i=1;i<=n;i++) cout<<hisab[scc[i]]<<endl;
        for(int i=1;i<=n;i++){
            int joy=dfs2(i);
            if(joy>k){
                k=joy;
