...
void bfs(pii home){
    ...
    while(!q.empty()){
        ...
        loop(i,4){
            ...
            if(valid(v.F,v.S)){
                if(s[v.F][v.S]=='#'||s[v.F][v.S]=='m') continue;
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        pii home;
        loop(i,r) loop(j,c){
            if(s[i][j]=='a') ob.a=pii(i,j);
            if(s[i][j]=='b') ob.b=pii(i,j);
            if(s[i][j]=='c') ob.c=pii(i,j);
            if(s[i][j]=='h') home=pii(i,j);
        }
        bfs(home);
        int ans=max(dis[ob.a.F][ob.a.S],max(dis[ob.b.F][ob.b.S],dis[ob.c.F][ob.c.S]));
        case_print;
        cout<<ans<<endl;
    }
    return 0;
