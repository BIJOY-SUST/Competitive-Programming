...
#define case_print          printf("Case %d:\n",cs)
...
int query(int node,int l,int r,int i,int j){
    ...
    int left=node<<1,right=(node<<1)+1,mid=(l+r)>>1;
    int ret1=query(left,l,mid,i,j);
    int ret2=query(right,mid+1,r,i,j);
    return ret1+ret2;
}
int main(){
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    int t,n,e,m,q,d,pos,tot,val;
    sfi1(t);
    loop1(cs,t){
        mem(arr,0);mem(tree,0);mem(dir,0);
        case_print;
        sfi2(n,q);
        loop1(i,n) sfi1(arr[i]),dir[i]=1;
        tot=n+q;
        build(1,1,tot);
        while(q--){
            getchar();
            ...
            if(ch=='a'){
                ...
                sfi1(pos);
                int l=1,r=n,mid,ans=0,cnt=0;
                while(cnt<31){
                    mid=(l+r)>>1;
                    int tem=query(1,1,tot,1,mid);
                    if(tem>=pos){
                        r=mid-1;
                        ans=mid;
                    }
