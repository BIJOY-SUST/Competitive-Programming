...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        int n;
        sfi1(n);
        loop1(i,n) sfi1(node[i]);
        loop1(i,n-1){
            edge[node[i]].pb(node[i+1]);
            edge[node[i+1]].pb(node[i]);
        }
        loop1(i,n) srt(edge[node[i]]);
        bfs(node[1],node[n]);
        case_print;
        pf("\n");
        path_print(node[1],node[n]);
        pf("\n");
    }
    return 0;
}
