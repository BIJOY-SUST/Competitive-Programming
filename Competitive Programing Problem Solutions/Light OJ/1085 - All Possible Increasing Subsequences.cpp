...
void build(int node,int l,int r,int pos,int val){
    ...
    if(l==r){
        if(l==pos) tree[node]=(tree[node]+(val%mod))%mod;
        return;
    }
    int left=node<<1,right=(node<<1)+1,mid=(l+r)>>1;
    build(left,l,mid,pos,val);
    build(right,mid+1,r,pos,val);
    tree[node]=((tree[node]%mod)+(val%mod))%mod;
    return;
}
int query(int node,int l,int r,int pos){
    if(r<=pos) return tree[node];
    else if(l>pos) return 0;
    int left=node<<1,right=(node<<1)+1,mid=(l+r)>>1;
    int ret1 = query(left,l,mid,pos);
    int ret2 = query(right,mid+1,r,pos);
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        int ans=0;
        loop1(i,n){
            int temp = query(1,1,n,ind[i])+1;
            ans=(ans+temp)%mod;
            build(1,1,n,ind[i],temp);
//            cout<<"i = "<<i<<" "<<ind[i]<<" "<<temp;
//            loop1(j,4*n){
//                cout<<"            j = "<<j<<" "<<tree[j]<<endl;
//            }
        }
        case_print;
        pf("%d\n",ans);
