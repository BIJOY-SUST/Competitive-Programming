...
int bfs(pii st,pii en){
    dis[st.F][st.S]=0;
    queue<pii>q;
    q.push(pii(st.F,st.S));
    while(!q.empty()){
        pii u=q.front();q.pop();
        if(u==en) return dis[u.F][u.S];
        loop(i,4){
            pii v = pii(u.F+fx[i],u.S+fy[i]);
            if(valid(v.F,v.S)){
                if(s[v.F][v.S]=='*'&&temp<=10){
                    temp++;
                    loop(i,star.size()){
                        pii v2=star[i];
                        if(v2.F==v.F&&v2.S==v.S){
                            if(dis[v2.F][v2.S]>dis[u.F][u.S]+3){
                                dis[v2.F][v2.S]=dis[u.F][u.S]+3;
                                q.push(pii(v2.F,v2.S));
                            }
                        }
                        else if(dis[v2.F][v2.S]>dis[u.F][u.S]+2){
                            dis[v2.F][v2.S]=dis[u.F][u.S]+2;
                            q.push(pii(v2.F,v2.S));
                        }
                    }
                }
                else if(s[v.F][v.S]!='#'){
                    if(dis[v.F][v.S]>dis[u.F][u.S]+1){
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        loop(i,r)loop(j,c){
            dis[i][j]=inf;
            if(s[i][j]=='P') st = pii(i,j);
            if(s[i][j]=='D') en = pii(i,j);
            if(s[i][j]=='*') star.pb(pii(i,j));
        }
        if(star.size()==1) dis[star[0].F][star[0].S]=-1;
        int ans=bfs(st,en);
        case_print;
