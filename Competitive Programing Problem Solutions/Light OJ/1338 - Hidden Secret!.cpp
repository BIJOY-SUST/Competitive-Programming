...
#define mpcll           unordered_map<char,ll>
...
#define forg(a,b)       for(ll i=a;i<b;i++)
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    for(int cs=1;cs<=t;cs++){
        gets(s);
        gets(ss);
        forg(0,strlen(s)) mp1[tolower(s[i])]++;
        forg(0,strlen(ss)) mp2[tolower(ss[i])]++;
        bool f=true;
        for(char ch='a';ch<='z';ch++){
            if(mp1[ch]!=mp2[ch]) {f=false;break;}
        }
        if(f)
            printf("Case %d: Yes\n",cs);
