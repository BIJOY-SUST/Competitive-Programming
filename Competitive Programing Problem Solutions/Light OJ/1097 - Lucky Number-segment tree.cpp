...
void update(int node,int l,int r,int val){
    ...
    int left=node<<1,right=(node<<1)+1,mid=(l+r)>>1;
    if(tree[left]>=val) update(left,l,mid,val);
    else update(right,mid+1,r,val-tree[left]);
    tree[node]=tree[left]+tree[right];
}
int query(int node,int l,int r,int val){
    if(l==r) return l;
    int left=node<<1,right=(node<<1)+1,mid=(l+r)>>1;
    if(tree[left]>=val) return query(left,l,mid,val);
    else return query(right,mid+1,r,val-tree[left]);
}
int main(){
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    ans[1]=1;
    for(int i=2;i<mx;i++){
        int val=query(1,1,inf,i);
        ans[i]=val;
        ///j=inf it's sometimes causes overflow so we should use j=(inf/val)*val...
        for(int j=(inf/val)*val;j>=val;j-=val) update(1,1,inf,j);
    }
    int t;
    sfi1(t);
    loop1(cs,t){
