...
map<int,int>par;
int find_par(int x){
    return (par[x]==x)? par[x]:find_par(par[x]);
}
ll mst(int n){
    ...
    for(int i=0;i<edge.size();i++){
        int pu=find_par(edge[i].u);
        int pv=find_par(edge[i].v);
        if(pu!=pv){
            par[pv]=pu;
            ans+=edge[i].w;
            fi_edge[edge[i].u].pb(edge[i].v);
            fi_edge[edge[i].v].pb(edge[i].u);
        }
    }
    return ans;
}
void dfs(int st){
    vis[st]=1;
    forstl(v,fi_edge[st]) if(!vis[*v]) dfs(*v);
    return;
}
int main(){
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        ans=area*air+ans;
        pf("%lld %d\n",ans,area);
        edge.clear();par.clear();
        loop(i,mx) fi_edge[i].clear();
    }
    return 0;
}
/*
