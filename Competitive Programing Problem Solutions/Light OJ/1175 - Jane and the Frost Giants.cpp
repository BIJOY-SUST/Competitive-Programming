...
string graph[210];
int r,c;
int vis[210][210],dis[210][210],dis2[210][210];
vpii vp;
void boss(){
    loop(i,r)loop(j,c){
    ...
    while(!q.empty()){
        ...
        loop(i,4){
            ...
            if(valid(v.F,v.S)&&vis[v.F][v.S]==0&&graph[v.F][v.S]=='.'){
                vis[v.F][v.S]=1;
...
int bfs(int st,int en){
    ...
    while(!q.empty()){
        pii u=q.front();q.pop();
        if(u.F==0||u.F==r-1||u.S==0||u.S==c-1) return dis2[u.F][u.S];
        loop(i,4){
            pii v = pii(u.F+fx[i],u.S+fy[i]);
            if(valid(v.F,v.S)&&vis[v.F][v.S]==0&&graph[v.F][v.S]=='.'&&dis[v.F][v.S]>dis2[u.F][u.S]+1){
                vis[v.F][v.S]=1;
                dis2[v.F][v.S]=dis2[u.F][u.S]+1;
                q.push(v);
            }
        }
    }
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        cin>>r>>c;
        loop(i,r) cin>>graph[i];
        int st,en;
        loop(i,r) loop(j,c){
            if(graph[i][j]=='F') vp.pb({i,j});
            if(graph[i][j]=='J'){
                st=i;
                en=j;
            }
