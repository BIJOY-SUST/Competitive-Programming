...
int bfs(int st){
    while(!q.empty()){
        int v=q.front(); q.pop();
        if(v==st) return cost[st];
        int u,w;
        if(v==(int)'a') u=(int)'z',w=v+1;
        else if(v==(int)'z') u=v-1,w=(int)'a';
        else u=v-1,w=v+1;
        for(int i=0;i<2;i++){
            int p;
            if(i) p=w;
...
//        forstl(ss) cout<<*it<<endl;
        printf("Case %d: ",cs);
        int ans=0;
        if(ss.find(shuru)==ss.end()&&ss.find(finish)==ss.end()){
            for(int i=0;i<3;i++){
                while(!q.empty()) q.pop();
                mem(cost,0);mem(vis,0);
                q.push(shuru[i]);
                vis[(int)shuru[i]]=1;
                ans+=bfs((int)finish[i]);
            }
            cout<<ans<<endl;
        }
        else cout<<"-1\n";
