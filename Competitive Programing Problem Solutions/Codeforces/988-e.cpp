...
int boss(string s,char x,char y){
    if(s.find(x)==string::npos||s.find(y)==string::npos) return inf;
//    debug(x,y);
    int indy=-1,ind=s.size()-1,step=0;
    for(int i=ind;i>=0;i--){
        if(s[i]==y){
            indy=i;
            break;
    ...
    if(indx==-1) return inf;
    while(s[ind-1]!=x){
        swap(s[indx],s[indx+1]);
        indx++;step++;
    }
    if(ind==1) return step;
    loop(i,ind-1){
//        debug(i);
        if(s[i]!='0') return step+i;
    }
    return inf;
}
int main(){
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    int ans=inf;
    ans=min(ans,boss(s,'2','5'));
//    debug(ans);
    ans=min(ans,boss(s,'5','0'));
//        debug(ans);

    ans=min(ans,boss(s,'7','5'));
//        debug(ans);

    ans=min(ans,boss(s,'0','0'));
//        debug(ans);
