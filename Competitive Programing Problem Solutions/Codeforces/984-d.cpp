...
///code start here
int t,n,dp[5010][5010];
int main(){
	cin>>n;
	loop1(i,n) cin>>dp[i][i];
	for(int i=n;i>=1;i--)for(int j=i+1;j<=n;j++)dp[i][j]=dp[i+1][j]^dp[i][j-1];
	for(int i=n;i>=1;i--)for(int j=i+1;j<=n;j++)dp[i][j]=max(dp[i][j],max(dp[i+1][j],dp[i][j-1]));
	cin>>t;
	int l,r;
	while(t--){
		cin>>l>>r;
...
int getXorUtil(int *st, int ss, int se, int qs, int qe, int si)
{
    ...
    int ret1=getXorUtil(st, ss, mid, qs, qe, 2*si+1);
    int ret2=       getXorUtil(st, mid+1, se, qs, qe, 2*si+2);
    sum=max(sum,ret1^ret2);
    return ret1^ret2;
}
...
int constructSTUtil(int arr[], int ss, int se, int *st, int si)
{
    ...
    int mid = getMid(ss, se);
    int p1=constructSTUtil(arr, ss, mid, st, si*2+1);
    int p2=constructSTUtil(arr, mid+1, se, st, si*2+2);
    st[si] =p1^p2;
    return st[si];
}
