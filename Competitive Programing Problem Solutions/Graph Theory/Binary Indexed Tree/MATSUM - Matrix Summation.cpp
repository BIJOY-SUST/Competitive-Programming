    ...
    loop1(cs,t){
        ...
        while(1){
            ...
            if(temp[1]=='E'){
                ...
                sfi3(x,y,val);
                x++;y++;
                ll p_val=query(x,y)-query(x,y-1)-query(x-1,y)+query(x-1,y-1);
                ///we also use data array to store (x,y) point value as if we find p_val value from the data array
                ///ll p_val=data[x][y];
                update(x,y,val-p_val);
            }
            else if(temp[1]=='U'){
                int x1,y1,x2,y2;
                sfi4(x1,y1,x2,y2);
