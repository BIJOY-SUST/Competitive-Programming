...
vll edge[mx];
vll ami,tumi;
ll vis[mx],low[mx],room[mx],disc[mx],cnt[mx],deep2[mx],deep[mx];
ll scc_no,c,root,ans[mx];
ll tarjan(ll u,ll p){
    ll prev=cnt[room[u]]-deep[u];
    ll pore=0;
    deep2[u]=1;
    low[u]=disc[u]=++c;
    for(auto v:edge[u]){
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        loop(i,mx){
            edge[i].clear();
            vis[i]=disc[i]=ans[i]=cnt[i]=deep2[i]=deep[i]=0;
            scc_no=0;c=0;
        }
        sfll2(n,e);
        loop1(i,e){
        ...
        loop1(i,n){
            if(disc[i]==0){
                ami.clear();tumi.clear();
                root=i;
                d=tarjan(i,-1);
                loop(j,ami.size()){
                    ans[ami[j]]+=(cnt[room[i]]-1);
                }
            }
        }
        _case;
