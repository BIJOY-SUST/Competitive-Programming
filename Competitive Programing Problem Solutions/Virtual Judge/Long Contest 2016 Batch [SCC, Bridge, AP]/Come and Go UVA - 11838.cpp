...
//struct data{int ind,val;};
//bool cmp(data x,data y){if(x.val==y.val) return x.ind<y.ind;else return x.val>y.val;}
///code start here
vi edge[mx];
sti st;
int low[mx],disc[mx],scc[mx],check[mx][mx];
bool vis[mx];
int cnt,cur_no;
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    int t,a,b,n,e,u,v,w,x,y,c,d,dir;
    while(cin>>n>>e){
        if(n==0&&e==0) break;
        loop(i,mx) edge[i].clear();
        mem(vis,0);mem(low,0);mem(disc,0);mem(check,0);
        cnt=0,cur_no=0;
        loop1(i,e){
            int u,v,dir;
            cin>>u>>v>>dir;
            if(dir==1) edge[u].pb(v);
            else{
                edge[u].pb(v);
                edge[v].pb(u);
            }
