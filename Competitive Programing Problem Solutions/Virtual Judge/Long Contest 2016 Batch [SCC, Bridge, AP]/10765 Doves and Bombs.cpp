...
///code start here
vi edge[mx];
int vis[mx],par[mx],low[mx],disc[mx],child[mx],cnt;
...
void dfs(int u,int root){
    ...
    for(auto v:edge[u]){
        if(!vis[v]){
            ...
            if(u==root&&children>1) child[u]=children-1;
            if(u!=root&&low[v]>=disc[u]) child[u]++;
        }
