...
Finding the minimum number of edges to add to a given graph, the entire graph is strongly connected.
analysis:

Analyze the nature of strong connectivity graphs to see that all points have an in-degree and out-degree of at least 1
If you want this graph to be connected, just after scc shrinks, for the new graph, find the number of roots (in degrees 0) and the number of leaves (degree 0), connect the root and leaf, and the graph is connected
The answer is obviously ans=max(number of roots, number of leaves)
Need special judgment that the entire graph has been strong when the ans = 0
**/
...
//bool cmp(data x,data y){if(x.v==y.v) return x.u<y.u;else return x.v>y.v;}
///code start here

vi edge[mx];
sti st;
int low[mx],disc[mx],vis[mx],scc[mx];
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    loop1(cs,t){
        ...
        mem(low,0);mem(disc,0);mem(scc,0);mem(vis,0);
        mem(in_deg,0);mem(out_deg,0);
        int n,e;
        sfi2(n,e);
        loop1(i,e){
            int u,v;
