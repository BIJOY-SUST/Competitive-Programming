...
void bfs(){
    while(!q.empty()){
        pii u=q.front();q.pop();
        if(dis1[{u.F,u.S}]==d) continue;
        loop(i,4){
            pii v={u.F+fx[i],u.S+fy[i]};
            if(v.F>=0&&v.F<n&&v.S>=0&&v.S<m){
//                cout<<dis1[{v.F,v.S}]<<" "<<dis1[{u.F,u.S}]<<endl;
                if(dis1[{v.F,v.S}]>dis1[{u.F,u.S}]+1){
                    q.push({v.F,v.S});
                    vis[{v.F,v.S}]=1;
                    dis1[{v.F,v.S}]=dis1[{u.F,u.S}]+1;
//                    cout<<dis1[{v.F,v.S}]<<" "<<d<<endl;
                }
            }
        }
    }
...
//    }
    if(!vis[{st.F,st.S}]){
        ...
        while(!q.empty()){
            ...
            loop(i,4){
                pii v={u.F+fx[i],u.S+fy[i]};
                if(v.F>=0&&v.F<n&&v.S>=0&&v.S<m){
                    if(dis2[{v.F,v.S}]>dis2[{u.F,u.S}]+1&&vis[{v.F,v.S}]==0){
                        q.push({v.F,v.S});
    ...
    if(dis2[{en.F,en.S}]!=inf){
        cout<<dis2[{en.F,en.S}]<<endl;
    }
    else cout<<"-1\n";
    return 0;
}
