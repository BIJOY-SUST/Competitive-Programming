...
void replaceAll(string& org, const string& ami, const string& tumi) {
    size_t pos = 0;
    while((pos = org.find(ami, pos)) != string::npos) {
         org.replace(pos, ami.length(), tumi);
         pos += tumi.length();
    }
}
map<char,int>mp1;
