...
ld cal_arc_(PT p1,PT p2,ld r){
    ld A = dist2D_sqr(p1,p2);
    ld B = dist2D_sqr(p1,PT(0.0,0.0));
    ld C = dist2D_sqr(p2,PT(0.0,0.0));
    ld ang = acos((B+C-A)/(2.0*sqrt(B*C)));
    return r*ang;
}
ld cal_ang(ld A,ld B,ld C){
    ld ang = acos(((B*B)+(C*C)-(A*A))/(2.0*(B*C)));
    return ang;
}
///code start here
...
//    freopen("Input.txt","r",stdin); freopen("Output.txt","w",stdout);
    ...
    while(t--){
        PT a,b,c;
        ld r;
        sfd4(a.x,a.y,b.x,b.y);
        sfd1(r);
        c=PT(0.0,0.0);
        ld ans;
        vector<PT>bindu = CircleLineIntersection(a,b,c,r);
        if(bindu.size()<2) ans=dist2D(a,b);
        else if(!PointOnSegment(bindu[0],a,b) || !PointOnSegment(bindu[1],a,b)) ans=dist2D(a,b);
        else{
            ld d1=dist2D_sqr(c,a);
            ld d2=dist2D_sqr(c,b);

            ld t1=d1-sqr(r);
            ld t2=d2-sqr(r);

            vector<PT>vec1 = CircleCircleIntersection(c,a,r,sqrt(t1));
            vector<PT>vec2 = CircleCircleIntersection(c,b,r,sqrt(t2));
            ans=1e50;
//            ans=min(ans,dist2D(a,vec1[0])+dist2D(b,vec2[0])+cal_arc_(vec1[0],vec2[0],r));
//            ans=min(ans,dist2D(a,vec1[0])+dist2D(b,vec2[1])+cal_arc_(vec1[0],vec2[1],r));
//            ans=min(ans,dist2D(a,vec1[1])+dist2D(b,vec2[0])+cal_arc_(vec1[1],vec2[0],r));
//            ans=min(ans,dist2D(a,vec1[1])+dist2D(b,vec2[1])+cal_arc_(vec1[1],vec2[1],r));

//                ans=min(ans,tem);)
            for(int i=0;i<2;i++)for(int j=0;j<2;j++){
                ld tem=dist2D(a,vec1[i])+dist2D(b,vec2[j])+cal_arc_(vec1[i],vec2[j],r);
                ans=min(ans,tem);
            }
//            deb(ans);
        }
